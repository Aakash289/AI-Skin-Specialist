...
# Import the three core functions that power the app: analyzing image/video +
# text with Claude, converting the doctor's reply to speech, and transcribing
# the patient's recorded voice into text
from Brain_of_the_doctor import brain_of_the_doctor
from voice_of_the_doctor import convert_text_to_doctor_audio
from voice_of_the_user import transcribe_patient_voice

# Title shown in the browser tab / window for the Gradio app
APP_TITLE = "AI Skin Specialist"

# Custom CSS for the whole interface: fonts, colors, spacing, card layouts,
# and overrides for Gradio's default component styling so the app has its
# own branded look instead of the default Gradio theme
CSS = """
...
# This function runs when the user clicks "Analyze Concern". It ties together
# all three backend pieces: transcribe the patient's voice, send the text
# plus image/video to Claude for a doctor-style response, then convert that
# response into spoken audio to play back
def process_inputs(audio_filepath, image_filepath, video_filepath):
    # Require the patient to have recorded/uploaded audio before proceeding
    if not audio_filepath:
        raise gr.Error("Please record or upload your voice description first.")
    # Require at least one visual (image or video) before proceeding
    if not image_filepath and not video_filepath:
        raise gr.Error("Please upload a skin image or video before analysis.")

    # Step 1: turn the patient's spoken audio into text
    patient_text = transcribe_patient_voice(audio_filepath)
    # Step 2: send the transcribed text plus the image/video to Claude,
    # which returns a short doctor-style text response
    doctor_text = brain_of_the_doctor(
        patient_text=patient_text,
        image_filepath=image_filepath,
        video_filepath=video_filepath,
    )
    # Step 3: convert the doctor's text response into a spoken audio file
    doctor_audio = convert_text_to_doctor_audio(doctor_text)

    # Return all three outputs, which Gradio will route into the matching
    # output components defined further down (transcript box, response box, audio player)
    return patient_text, doctor_text, str(Path(doctor_audio))

# Build the actual Gradio interface layout using Blocks, which allows a fully
# custom arrangement of components (rather than a fixed simple layout)
with gr.Blocks(title=APP_TITLE) as iface:
    with gr.Column(elem_classes="ais-shell"):
        # Top header bar: app title, subtitle, and a small "privacy" badge
        gr.HTML(
            """
        ...
        # Two-column layout: patient input on the left, doctor response on the right
        with gr.Row(elem_classes="ais-grid"):
            # Left column: everything the patient provides as input
            with gr.Column(scale=5):
                ...
                with gr.Column(elem_classes="ais-card ais-input-card"):
                    ...
                    # Microphone/upload widget for the patient's spoken description
                    audio_input = gr.Audio(
                        sources=["microphone", "upload"],
                        type="filepath",
                        label="Patient Voice",
                    )
                    # Side-by-side image and video upload widgets
                    ...
                    # The main button that triggers the whole analysis pipeline
                    ...
                    # Helper tip shown under the button
                    gr.HTML(
                        """
                        <div class="ais-note">
            ...
            # Right column: where the doctor's response gets displayed after analysis
            with gr.Column(scale=7):
                ...
                with gr.Column(elem_classes="ais-card ais-response-card"):
                    # Placeholder message shown before any analysis has been run
                    gr.HTML(
                        """
                        <div class="ais-empty">
                    ...
                    # The three output components that get filled in once
                    # process_inputs() runs: what the patient said, what the
                    # doctor (Claude) replied, and the doctor's reply as audio
                ...
                # Disclaimer shown below the response card
                gr.HTML(
                    """
        ...
        # Footer with app name and legal/disclaimer links
        gr.HTML(
            """
    ...
    # Wire the button click to actually run process_inputs(), mapping the
    # three input widgets to the function's parameters, and the function's
    # three return values to the three output widgets
...
# Only launch the app when this file is run directly (not when imported),
# starting the local Gradio web server with the custom CSS and a clean base theme
